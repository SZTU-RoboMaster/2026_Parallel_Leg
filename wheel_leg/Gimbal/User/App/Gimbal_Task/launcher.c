...
static void fir_wheel_control(void)
{
    ...
    /** 摩擦轮电流计算 **/
    launcher.fire_l.state = update_system(launcher.fire_l.target_speed, launcher.fire_l.motor_measure.speed_rpm, 0.001f, launcher.fire_l.state);
    launcher.fire_l.target_current = -smc_controller(launcher.fire_l.state, params);

    launcher.fire_r.state = update_system(launcher.fire_r.target_speed, launcher.fire_r.motor_measure.speed_rpm, 0.001f, launcher.fire_r.state);
    launcher.fire_r.target_current = -smc_controller(launcher.fire_r.state, params);
