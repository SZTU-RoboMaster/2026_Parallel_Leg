...
void Detect_task(void const*pvParameters){
    ...
    for(uint8_t i=DETECT_CHASSIS_3508_RF;i<=DETECT_CHASSIS_3508_RB;i++)
        detect_init(i,500,2);//200ms

    //1级的话未必要等级,1级以上需要控制车不要发癫
    detect_init(DETECT_REFEREE,500,1);
    detect_init(DETECT_AUTO_AIM,500,1);
    detect_init(DETECT_CAP,800,1);

    detect_init(DETECT_LAUNCHER_3508_FIRE_L,500,3);
    detect_init(DETECT_LAUNCHER_3508_FIRE_R,500,3);
    detect_init(DETECT_LAUNCHER_3508_TRIGGER,500,3);

    detect_init(DETECT_GIMBAL_6020_PITCH,500,4);
    detect_init(DETECT_GIMBAL_6020_YAW,500,4);

    detect_init(DETECT_REMOTE,1000,5);
    detect_init(DETECT_VIDEO_TRANSIMITTER,10000,5);
