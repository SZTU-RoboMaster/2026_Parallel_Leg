...
void update_pc_info() {
    ...
    if(control_flag == ALL_ONLINE) //都在线,优先遥控器
    {
        ...
        /* 图传优先 */
        // key_update(&KeyBoard.W, Referee_Task.keyboard.keyboard_value & KEY_W, DEFAULT_CNT);//DEFAULT_CNT 为按键检测为长按的默认时间
        // key_update(&KeyBoard.A, Referee_Task.keyboard.keyboard_value & KEY_A, DEFAULT_CNT);
        // key_update(&KeyBoard.S, Referee_Task.keyboard.keyboard_value & KEY_S, DEFAULT_CNT);
        // key_update(&KeyBoard.D, Referee_Task.keyboard.keyboard_value & KEY_D, DEFAULT_CNT);
        // key_update(&KeyBoard.Q, Referee_Task.keyboard.keyboard_value & KEY_Q, DEFAULT_CNT);
        // key_update(&KeyBoard.E, Referee_Task.keyboard.keyboard_value & KEY_E, DEFAULT_CNT);
        // key_update(&KeyBoard.R, Referee_Task.keyboard.keyboard_value & KEY_R, DEFAULT_CNT);
        // key_update(&KeyBoard.F, Referee_Task.keyboard.keyboard_value & KEY_F, DEFAULT_CNT);
        // key_update(&KeyBoard.G, Referee_Task.keyboard.keyboard_value & KEY_G, DEFAULT_CNT);
        // key_update(&KeyBoard.CTRL, Referee_Task.keyboard.keyboard_value & KEY_CTRL, DEFAULT_CNT);
        // key_update(&KeyBoard.SHIFT, Referee_Task.keyboard.keyboard_value & KEY_SHIFT, DEFAULT_CNT);
        // key_update(&KeyBoard.B, Referee_Task.keyboard.keyboard_value & KEY_B, DEFAULT_CNT);
        // key_update(&KeyBoard.C, Referee_Task.keyboard.keyboard_value & KEY_C, DEFAULT_CNT);
        // key_update(&KeyBoard.V, Referee_Task.keyboard.keyboard_value & KEY_V, DEFAULT_CNT);
        // key_update(&KeyBoard.X, Referee_Task.keyboard.keyboard_value & KEY_X, DEFAULT_CNT);
        // key_update(&KeyBoard.Z, Referee_Task.keyboard.keyboard_value & KEY_Z, DEFAULT_CNT);
        //
        // /*鼠标部分*/
        //  key_update(&KeyBoard.Mouse_l, Referee_Task.keyboard.left_button_down  & MOUSE_YES, MOUSE_CLICK_L_CNT);
        //  key_update(&KeyBoard.Mouse_r, Referee_Task.keyboard.right_button_down & MOUSE_YES, MOUSE_CLICK_R_CNT);
    }
    ...
    {
        if(KeyBoard.B.status==KEY_PRESS)//按B键失能
        {
            rc_ctrl.rc.s[RC_s_L] = RC_SW_DOWN;
            rc_ctrl.rc.s[RC_s_R] = RC_SW_DOWN;
        }
        if(KeyBoard.G.status==KEY_PRESS)//按G键使能
        {
